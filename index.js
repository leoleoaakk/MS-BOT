...
class Music {
  ...
  async play(msg) {
    ...
    if (!this.connection[guildID]) {
      msg.channel.send(`請先將機器人 ${prefix[1]}join 加入頻道`);
      return;
    }

    // 如果 Bot leave 後又未加入語音頻道
    if (this.connection[guildID].status === 4) {
      msg.channel.send(`請先將機器人 ${prefix[1]}join 重新加入頻道`);
      return;
    }

    // 處理字串，將 !!play 字串拿掉，只留下 YouTube 網址
    const musicURL = msg.content.replace(`${prefix[1]}play`, '').trim();
...
client.on('message', async (msg) => {
  ...
  // 如果使用者輸入的內容中包含 !!play
  if (msg.content.indexOf(`${prefix[1]}play`) > -1) {
  ...
  // !!resume
  if (msg.content === `${prefix[1]}resume`) {
